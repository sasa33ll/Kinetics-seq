...
# %%
adata= sc.read_h5ad('cancercell_adata.h5ad')
...
alpha_matrix_strict = np.where(high_conf_mask, alpha_matrix, np.nan)
gamma_matrix_strict = np.where(high_conf_mask, gamma_matrix, np.nan)
hl_matrix_strict = np.where(high_conf_mask, hl_matrix, np.nan)
with warnings.catch_warnings():
    warnings.simplefilter("ignore", category=RuntimeWarning)
    adata.var['alpha_gene_cancer'] = np.nanmean(alpha_matrix, axis=0)
    adata.var['gamma_gene_cancer'] = np.nanmean(gamma_matrix, axis=0)
    adata.var['half_life_gene_cancer'] = np.nanmean(hl_matrix, axis=0)
    adata.var['alpha_gene_strict_cancer'] = np.nanmean(alpha_matrix_strict, axis=0)
    adata.var['gamma_gene_strict_cancer'] = np.nanmean(gamma_matrix_strict, axis=0)
    adata.var['half_life_gene_strict_cancer'] = np.nanmean(hl_matrix_strict, axis=0)
...
# with warnings.catch_warnings():
#     warnings.simplefilter("ignore", category=RuntimeWarning)
#     adata.var['alpha_gene_strict'] = np.nanmean(alpha_matrix_strict, axis=0)
#     adata.var['gamma_gene_strict'] = np.nanmean(gamma_matrix_strict, axis=0)
#     adata.var['half_life_gene_strict'] = np.nanmean(hl_matrix_strict, axis=0)

adata.write_h5ad('cancercell_kinetics_adata.h5ad')
